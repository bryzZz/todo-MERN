            ...
            <div className="list-group" style={{borderRadius: '0'}}>
                {
                    items.map((list, index) => {
                        return (
                            <div className="d-flex" key={ list._id }>
                                <Link
                                    to={`${url}/${list._id}`}
                                ...
                                <button onClick={e => onDelete(list._id)} className="btn btn-outline-secondary">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" stroke="#fff" fill="currentColor" className="bi bi-trash" viewBox="0 0 16 16">
